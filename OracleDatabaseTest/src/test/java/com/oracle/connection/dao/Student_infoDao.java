...
public class Student_infoDao {
	...
	public static void addStudent(Student_info student_info) {
		...
		try {
			...
			statement = conn.prepareStatement("insert into student_info values(?,?,?,to_date(?,'mm-dd-yyyy'),?)");
			...
			statement.setString(4, student_info.getBirth_date());
			statement.setLong(5, student_info.getAge());
